...
\begin{frame}[fragile]
  ...
  %% Explicación de (contra)covarianza

  \begin{lstlisting}[language=scala, caption=Definición del ADT Opción]
...
\begin{frame}[fragile]
  ...
  \begin{lstlisting}[language=scala, caption=Ejercicio: Implementación
    del método map]
sealed trait Opcion[+A]
...
\begin{frame}[fragile]
  ...
  \begin{lstlisting}[language=scala, caption=Definición del ADT Lista]
...
\begin{frame}[fragile]
  ...
  \begin{block}{}
    Sabemos lo que es una función pura y somos capaces de formalizar
    la idea mediante \emph{referential transparency} y
    \emph{substitution model}.
  \end{block}
  \begin{block}{}
    Hemos descubierto las funciones en Scala, junto con otras técnicas
    y mecanismos para lidiar con ellos (listas de argumentos,
    currificación, expresiones lambda\ldots)
  \end{block}
  \begin{block}{}
  ...
  \end{block}
  \begin{block}{}
    Empezamos a apreciar la noción de \emph{``follow the type''}.
  \end{block}
\end{frame}

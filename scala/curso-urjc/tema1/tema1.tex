...
%% Clase Curso (option, parámetro por defecto)

%% Clase Gestión (nuestro primer método)

%% Pruebas (vistazo general al DSL ScalaTest)
...
%%   \begin{block}{Declaración de una Función}
%%     \begin{itemize}
%%     \item Esta función inscribe a un alumno en un curso, siempre que
%%       la acción sea posible.
%%     \item No es necesario entender todo el código. Sólo se pretende
%%       dar un primer vistazo a una función de Scala.
%%     \end{itemize}
%%   \end{block}

%%   \begin{lstlisting}[language=scala, caption=Vistazo a una función]
%% def inscribirAlumno(al: Alumno, crs: Curso): Option[Gestion] = {
%%   val alumnos = relacion(crs)
%%   if ((alumnos.size >= crs.limiteAlumnos) 
%%       || alumnos.contains(al))
%%     None
%%   else
%%     Some(new Gestion(relacion + (crs -> (al :: alumnos))))
%% }
%%   \end{lstlisting}
%% \end{frame}
